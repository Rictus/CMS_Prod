...
gulp.task('css', function () {
    var stream = gulp.src("./themes/default/css/**/*.less")
        .pipe(less())
        .pipe(autoprefixer('> 1%'));
    if (conf.minify) {
        stream = stream.pipe(minify());
    }
    return stream.pipe(gulp.dest("./themes/default/css/"));
});

var lessWatcher = gulp.watch("./themes/default/css/**/*.less", ['css']);
...
gulp.task('img', function () {
    return gulp.src('./themes/default/imgDev/**/*.{png,jpg,jpeg,gif,svg}')
        .pipe(imagemin())
        .pipe(gulp.dest('./themes/default/img/'));
});

var imgWatcher = gulp.watch("./themes/default/imgDev/**/*.{png,jpg,jpeg,gif,svg}", ['img']);
